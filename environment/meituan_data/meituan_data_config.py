...
class MeituanDataConfig:
    """Configuration for loading and applying Meituan data to simulations with vehicle scaling."""
    ...
    def __init__(
        ...
        scale_vehicles_to_restaurants: bool = True,  # Enable vehicle scaling
        vehicles_per_restaurant_ratio: float = 0.54,  # Target ratio: 0.54 couriers per restaurant
    ):
        self.district_id = district_id
        self.day = day
        self.simulation_start_hour = simulation_start_hour
        ...
        # NEW: Vehicle scaling parameters
        self.scale_vehicles_to_restaurants = scale_vehicles_to_restaurants
        self.vehicles_per_restaurant_ratio = vehicles_per_restaurant_ratio
    ...
    def get_scaled_vehicle_count(self) -> int:
        """
        Get vehicle count scaled to restaurant count using the specified ratio.

        Returns:
            int: Scaled number of vehicles (minimum 1)
        """
        if not self.scale_vehicles_to_restaurants:
            # Use original vehicle count if scaling is disabled
            return self.get_vehicle_count() or 1

        restaurant_count = self.get_restaurant_count()
        if restaurant_count is None:
            # Fallback to original vehicle count if restaurant data unavailable
            return self.get_vehicle_count() or 1

        # Calculate scaled vehicle count
        scaled_count = int(round(restaurant_count * self.vehicles_per_restaurant_ratio))

        # Ensure at least 1 vehicle
        return max(1, scaled_count)
    ...
    #         updated_params["service_area_dimensions"] = (self.geo_bounds["width_km"], self.geo_bounds["height_km"])
    ...
    #             updated_params["cooldown_duration"], updated_params["simulation_duration"] * cooldown_ratio
    #         )

    #     return updated_params

    def apply_to_env_params(self, env_params: dict) -> dict:
        """Apply configuration to environment parameters with vehicle scaling."""
        updated_params = env_params.copy()
        ...
        # Update vehicle count with scaling
        if self.use_vehicle_count:
            if self.scale_vehicles_to_restaurants:
                # Use scaled vehicle count
                scaled_count = self.get_scaled_vehicle_count()
                updated_params["num_vehicles"] = scaled_count

                # Log the scaling for transparency
                restaurant_count = self.get_restaurant_count()
                original_count = self.get_vehicle_count()
                logger.info(
                    f"Vehicle scaling: {restaurant_count} restaurants × {self.vehicles_per_restaurant_ratio:.2f} = {scaled_count} vehicles (original: {original_count})"
                )
            else:
                # Use original vehicle count
                ...
                if self.simulation_start_hour is not None:
                    try:
                        ...
                        if not pd.api.types.is_datetime64_any_dtype(filtered_orders_df["order_push_time"]):
                            filtered_orders_df["order_push_time"] = pd.to_datetime(
                                filtered_orders_df["order_push_time"]
                            )
