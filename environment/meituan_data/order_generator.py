...
class OrderGenerator:
    ...
    """
    def __init__(self, mean_interarrival_time=2.0, service_area_dimensions=(10.0, 10.0),
                 delivery_window=40.0, service_time=2.0, mean_prep_time=10.0, prep_time_var=2.0,
                 mode="default", temporal_pattern=None, real_orders_df=None):
        """
    ...
    def _preprocess_real_orders(self):
        """Prepare real order data for replay"""
        # Sort by order time
        ...
        logger.info(f"First 3 order times: {self.real_orders_df['sim_time'].head(3).tolist()}")
        logger.info(f"Average orders per hour: {len(self.real_orders_df) / (self.real_orders_df['sim_time'].max() / 60):.1f}")
    ...
    def _create_order_from_real_data(self, order_row, current_time, restaurants, geo_bounds):
        ...
        try:
            # if pd.notna(order_row.get('estimate_meal_prepare_time')) and pd.notna(order_row.get('order_push_time')):
            #     # Handle different datetime formats
            #     if isinstance(order_row['estimate_meal_prepare_time'], (int, float)):
            #         # For Unix timestamps
            #         real_prep_time = (order_row['estimate_meal_prepare_time'] - order_row['order_push_time']) / 60
            #     else:
            #         # For pandas Timestamp objects
            #         real_prep_time = (order_row['estimate_meal_prepare_time'] - 
            #                         order_row['order_push_time']).total_seconds() / 60
                    
            #     prep_time = max(1.0, min(60.0, real_prep_time))  # Limit to reasonable range
            # else:
            #     # Use default
            #     prep_time = np.random.gamma(
            #         shape=(self.mean_prep_time**2) / self.prep_time_var,
            ...
            logger.warning(f"Error calculating prep time: {e}. Using default.")
            prep_time = self.mean_prep_time
