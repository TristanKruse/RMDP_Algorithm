...
def prepare_solver_input(state: State) -> dict:
    ...
    # Log the orders_info to verify postpone_count
    # logger.info(f"Time {state.time:.2f}: Prepared orders_info: {orders_info}")
...
def get_solver_params(solver):
    ...
    attribute_list = [
        ...
        "postponement_method",
        "rl_training_mode",
        "rl_state_size",  # Add RL attributes
    ]
    ...
    if hasattr(solver, "postponement_method") and solver.postponement_method == "rl":
        ...
        if hasattr(solver.postponement, "batch_losses") and solver.postponement.batch_losses:
            params["rl_avg_loss"] = sum(solver.postponement.batch_losses[-100:]) / min(
                100, len(solver.postponement.batch_losses)
            )
...
#     # Use the provided solver if available, otherwise create a new one
#     if solver is None:
#         solver = SOLVERS[solver_name](movement_per_step, env.location_manager)
...
#         solver = SOLVERS[solver_name](movement_per_step, env.location_manager)
#         # Load RL model if path provided (only if creating a new solver)
#         if rl_model_path and hasattr(solver, 'postponement_method') and solver.postponement_method == "rl":
#             solver.postponement.load_model(rl_model_path)
...
def run_test_episode(
    ...
    simulation_duration = simulation_duration = get_env_config(None)["simulation_duration"]  # 420
    speed = 16  # 40.0 km/h in paper, 16 km/h in Meituan data
    street_network_factor = (
        1.0  # 1.4 in paper, we calculated the average speed over the euclidic distance, so no adjustment necessary
    )
    movement_per_step = (speed / 60) / street_network_factor  # km per minute adjusted for street network
    ...
    # Load RL model if path provided
    if rl_model_path and hasattr(solver, "postponement_method") and solver.postponement_method == "rl-aca":
        print("block is called")
        # Verify that the model file exists
        if not os.path.exists(rl_model_path):
            logger.error(f"RL model file does not exist at {rl_model_path}. Exiting.")
            raise FileNotFoundError(f"RL model file not found: {rl_model_path}")
        # Load the model
        solver.postponement.load_model(rl_model_path)
        # Verify model loading by checking the state dict
        try:
            import torch

            expected_state_dict = torch.load(rl_model_path)
            actual_state_dict = solver.postponement.value_network.state_dict()
            # Compare a few keys to confirm the model was loaded
            matching = all(
                torch.equal(expected_state_dict[key], actual_state_dict[key])
                for key in expected_state_dict.keys()
                if key in actual_state_dict
            )
            if not matching:
                logger.warning("Model weights do not match the loaded model. Model may not have been loaded correctly.")
            else:
                logger.info("Model weights verified: Loaded model matches expected weights.")
    ...
    # Additional check for RL-ACA to confirm model loading expectations
    if solver_name == "rl_aca":
        if rl_model_path:
            if training_mode:
                logger.info(
                    f"Training mode enabled for RL-ACA solver. Model at {rl_model_path} will not be loaded again as it should already be handled."
                )
            else:
                logger.info(
                    f"Evaluation mode enabled for RL-ACA solver. Model should have been loaded from {rl_model_path}."
                )
        else:
            if not training_mode:
                logger.warning(
                    "Evaluation mode enabled for RL-ACA solver, but no rl_model_path provided. Using default weights."
                )
            ...
            for order_id in info.get("delivered_orders", set()):
                ...
                if delivered_order:
                    ...
                    solver.postponement.record_order_delivery(
                        order_id, actual_delay, state.time, was_bundled=was_bundled
                    )
                    # Log the delivery
                    # print(f"Recorded delivery for order {order_id} with final delay: {actual_delay:.2f}, was_bundled: {was_bundled}")
            ...
            for order_id in info.get("delivered_orders", set()):
                ...
                if delivered_order:
                    ...
                    if order_id in predicted_delays:
                        ...
                        logger.info(
                            f"DELAY-ACTUAL: Order {order_id} - Predicted: {pred:.2f}, Actual: {actual_delay:.2f}, "
                            + f"Error: {error:.2f} ({error_percent:.1f}%)"
                        )
    ...
    if save_results_to_disk:
        ...
        if episode_stats["bundles_formed"] > 0:
            ...
            episode_stats["bundling_rate"] = (bundled_orders_count / max(1, episode_stats["orders_delivered"])) * 100
            episode_stats["avg_bundle_size"] = sum(episode_stats["bundle_sizes"]) / len(episode_stats["bundle_sizes"])
            episode_stats["same_restaurant_bundle_rate"] = (
                episode_stats["same_restaurant_bundles"] / episode_stats["bundles_formed"]
            ) * 100

            # Bundle efficiency
            if episode_stats["bundle_delays"] and episode_stats["non_bundle_delays"]:
                episode_stats["avg_bundle_delay"] = sum(episode_stats["bundle_delays"]) / len(
                    episode_stats["bundle_delays"]
                )
                episode_stats["avg_non_bundle_delay"] = sum(episode_stats["non_bundle_delays"]) / len(
                    episode_stats["non_bundle_delays"]
                )
                episode_stats["bundle_delay_impact"] = (
                    episode_stats["avg_bundle_delay"] - episode_stats["avg_non_bundle_delay"]
                )

        # ----- KPI Tracking -----
        # Calculate averages for all time metrics
        if episode_stats["true_prep_times"]:
            episode_stats["avg_true_prep_time"] = sum(episode_stats["true_prep_times"]) / len(
                episode_stats["true_prep_times"]
            )

        if episode_stats["order_wait_times"]:
            episode_stats["avg_order_wait_time"] = sum(episode_stats["order_wait_times"]) / len(
                episode_stats["order_wait_times"]
            )

        if episode_stats["total_pickup_times"]:
            episode_stats["avg_total_pickup_time"] = sum(episode_stats["total_pickup_times"]) / len(
                episode_stats["total_pickup_times"]
            )

        # Calculate average times by restaurant
        episode_stats["avg_true_prep_by_restaurant"] = {
            r_id: sum(times) / len(times) for r_id, times in episode_stats["true_prep_by_restaurant"].items()
        }

        episode_stats["avg_order_wait_by_restaurant"] = {
            r_id: sum(times) / len(times) for r_id, times in episode_stats["order_wait_by_restaurant"].items()
        }

        episode_stats["avg_total_pickup_by_restaurant"] = {
            r_id: sum(times) / len(times) for r_id, times in episode_stats["total_pickup_by_restaurant"].items()
        }

        episode_stats["avg_driver_wait_by_restaurant"] = {
            r_id: sum(times) / len(times) for r_id, times in episode_stats["driver_wait_by_restaurant"].items()
        }
        ...
        late_rate = (late_orders / delivered_orders * 100) if delivered_orders else 0
        avg_delay = (
            sum(episode_stats["delay_values"]) / len(episode_stats["delay_values"])
            if episode_stats["delay_values"]
            else 0
        )
        avg_distance = (episode_stats["total_distance"] / delivered_orders) if delivered_orders else 0
        ...
        if episode_stats["orders_per_restaurant"]:
            # Sort restaurants by order count (descending)
            sorted_restaurants = sorted(
                episode_stats["orders_per_restaurant"].items(), key=lambda x: x[1], reverse=True
            )
            logger.info(f"Total restaurants served: {len(episode_stats['orders_per_restaurant'])}")
            logger.info(
                f"Restaurant with most orders: ID {sorted_restaurants[0][0]} with {sorted_restaurants[0][1]} orders"
            )
            ...
            for i, (r_id, count) in enumerate(sorted_restaurants[:5], 1):
                logger.info(
                    f"  #{i}: Restaurant {r_id} - {count} orders ({count/delivered_orders*100:.1f}% of all deliveries)"
                )
        ...
        if episode_stats["true_prep_times"]:
            ...
            if episode_stats["avg_true_prep_by_restaurant"]:
                logger.info("\n  Top 5 Restaurants by True Preparation Time:")
                top_prep_restaurants = sorted(
                    episode_stats["avg_true_prep_by_restaurant"].items(), key=lambda x: x[1], reverse=True
                )[:5]
        ...
        if episode_stats["order_wait_times"]:
            ...
            if episode_stats["avg_order_wait_by_restaurant"]:
                logger.info("\n  Top 5 Restaurants by Order Wait Time:")
                top_wait_restaurants = sorted(
                    episode_stats["avg_order_wait_by_restaurant"].items(), key=lambda x: x[1], reverse=True
                )[:5]
        ...
        logger.info(f"  Total Driver Wait Time: {episode_stats['total_driver_wait_time']:.1f} minutes")
        logger.info(
            f"  Drivers Waited for {episode_stats['driver_wait_orders']} orders ({episode_stats['driver_wait_percentage']:.1f}%)"
        )
        ...
        if multi_order_restaurants:
            ...
            for restaurant_id in multi_order_restaurants[:3]:  # Limit to first 3 to avoid too much output
                ...
                if len(restaurant_orders) < 2:
                    for order in state.orders:
                        if (
                            hasattr(order, "pickup_node_id")
                            and order.pickup_node_id.id == restaurant_id
                            and order.id not in restaurant_orders
                        ):
                            restaurant_orders.append(order.id)
                            if len(restaurant_orders) >= 3:
                                break
...
def save_results(
    stats: Dict, solver_name: str, seed: Optional[int] = None, meituan_config=None, solver_params=None, env_params=None
):
    ...
    avg_orders_per_restaurant = (
        sum(restaurant_order_counts) / len(restaurant_order_counts) if restaurant_order_counts else 0
    )
    ...
    if stats["orders_per_restaurant"]:
        sorted_restaurants = sorted(stats["orders_per_restaurant"].items(), key=lambda x: x[1], reverse=True)
    ...
    if stats["avg_true_prep_by_restaurant"]:
        top_prep_restaurants = sorted(stats["avg_true_prep_by_restaurant"].items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
    ...
    if stats["avg_order_wait_by_restaurant"]:
        top_wait_restaurants = sorted(stats["avg_order_wait_by_restaurant"].items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
    ...
    save_data = {
        **stats,
        ...
        "same_restaurant_bundle_rate": (
            stats["same_restaurant_bundle_rate"] if "same_restaurant_bundle_rate" in stats else 0
        ),
    ...
    fixed_fieldnames = [
        ...
        "use_restaurant_positions",
        "use_vehicle_count",
        "use_vehicle_positions",
        "use_service_area",
        "use_deadlines",
        ...
        "env_prep_time_var",
        "env_delivery_window",
        "env_simulation_duration",
        "env_cooldown_duration",
        "env_mean_interarrival_time",
        "env_service_area_dimensions",
        "env_downtown_concentration",
        "env_service_time",
        "env_movement_per_step",
        "env_reposition_idle_vehicles",
        "env_seed",
        "env_update_interval",
        "env_demand_pattern",
        "env_visualize",
        ...
        "solver_buffer",
        "solver_max_postponements",
        "solver_max_postpone_time",
        "solver_vehicle_capacity",
        "solver_service_time",
        "solver_mean_prep_time",
        "solver_delivery_window",
        "solver_max_bundle_size",
        ...
        "orders_per_hour",
        "active_period_orders_per_hour",
        "system_capacity",
        "active_period_capacity",
        "on_time_delivery_rate",
        "percentage_late_orders",
        "avg_delay_late_orders",
        "avg_distance_per_order",
        # Restaurant metrics
        "total_restaurants_served",
        "avg_orders_per_restaurant",
        "min_restaurant_orders",
        "max_restaurant_orders",
        "top_restaurant_id",
        # Bundle metrics
        "bundles_formed",
        "bundling_rate",
        ...
        "avg_order_wait_time",
        "max_order_wait_time",
        "avg_total_pickup_time",
        "max_total_pickup_time",
        "total_driver_wait_time",
        "driver_wait_orders",
        "driver_wait_percentage",
        "Note",
        "",
    ]
...
def visualize_restaurant_distribution(stats, solver_name, timestamp):
    ...
    for bar in bars:
        ...
        plt.text(bar.get_x() + bar.get_width() / 2.0, height + 0.5, f"{height:.0f}", ha="center", va="bottom")
...
def analyze_bundling_opportunity(solver, state_dict, orders_to_check):
                ...
                if order.id == order_id:
                    ...
                    all_order_info[order_id] = {
                        ...
                        "deadline": (
                            ...
                            else (order.request_time + solver.time_calculator.delivery_window)
                        ),
                    }
                    break
    ...
    for i, order_id in enumerate(orders_to_check):
        ...
        individual_routes[vehicle_id] = Route(
            ...
            sequence=[(pickup_node_id, {order_id}, set()), (delivery_node_id, set(), {order_id})],
            total_distance=0.0,
            total_time=0.0,
        )
    ...
    individual_delay_no_buffer = solver.time_calculator._calculate_delay(test_state, individual_routes, 0.0)
    # Then with buffer
    individual_delay = solver.time_calculator._calculate_delay(test_state, individual_routes, solver.buffer)
    ...
    bundled_routes[2000] = Route(vehicle_id=2000, sequence=bundled_sequence, total_distance=0.0, total_time=0.0)
    ...
    bundled_delay_no_buffer = solver.time_calculator._calculate_delay(test_state, bundled_routes, 0.0)
    # Then with buffer
    bundled_delay = solver.time_calculator._calculate_delay(test_state, bundled_routes, solver.buffer)
    ...
    sequential_routes[3000] = Route(vehicle_id=3000, sequence=sequential_sequence, total_distance=0.0, total_time=0.0)
    ...
    sequential_delay_no_buffer = solver.time_calculator._calculate_delay(test_state, sequential_routes, 0.0)
    sequential_delay = solver.time_calculator._calculate_delay(test_state, sequential_routes, solver.buffer)
    ...
    logger.info(
        f"  Time savings from bundling vs sequential individual: {sequential_max_time - bundled_max_time:.2f} minutes"
    )
    ...
    logger.info(
        f"BUNDLE-ANALYSIS: Service time savings (bundled vs sequential): {sequential_vs_bundled_savings} minutes"
    )
...
if __name__ == "__main__":
    ...
    stats = run_test_episode(
        solver_name="rl_aca",  # aca, rl_aca, bundler, fastest
