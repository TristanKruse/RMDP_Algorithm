...
class RLPostponementDecision:
    ...
    def extract_state_features(self, order_id: int, route_plan: dict, current_time: float, state: dict) -> torch.Tensor:
        ...
        total_assigned = sum(
            sum(len(p) + len(d) for _, p, d in route.sequence) for route in route_plan.values() if route.sequence
        )
        ...
        if order_info:
            ...
            same_restaurant_orders = sum(
                1
                for o_id, o_info in state["unassigned_orders"].items()
        ...
        if self.training_mode:
            ...
            if should_postpone:
                ...
                self.replay_buffer.add(
                    state=state_tensor.unsqueeze(0), action=1, reward=self.postponement_penalty, delta_t=0
                )
    ...
    def _process_completed_orders(self, current_time=None):
        ...
        for order_id in self.completed_orders:
            ...
            normalized_reward = (
                -(final_delay / 300) if final_delay > 0 else self.on_time_reward
            )  # Use 300 as max delay for normalization
            ...
            if self.training_mode:
                for i, (state_tensor, action, action_time) in enumerate(order_data["actions"]):
                    ...
                    self.replay_buffer.add(
                        state=state_tensor.unsqueeze(0), action=action, reward=normalized_reward, delta_t=delta_t
                    )
                    experiences_added += 1
