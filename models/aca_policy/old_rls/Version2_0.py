...
class RLPostponementDecision:
    def __init__(
        ...
        exploration_decay: float = 0.999,  # Slower decay
        min_exploration_rate: float = 0.01,
        batch_size: int = 32,
        training_mode: bool = True,
        state_size: int = 6,
        lns_sample_size: int = 5,
        max_order_age: int = 300,  # Increased to reduce timeouts
        timeout_reward: float = -160.0  # Will be normalized
    ):
        ...
        self.target_network = PostponementValueNetwork(state_size)  # Add target network
