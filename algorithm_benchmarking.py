...
from training.train import run_test_episode, MeituanDataConfig, lunch_dinner_pattern
...
def benchmark_methods():
    ...
    episodes_per_run = {"fastest": 10, "aca": 10, "rl_aca": 10}
            ...
            for method in methods:
                ...
                for run in range(num_runs):
                    ...
                    for episode in range(num_episodes):
                        seed = (
                            (run * num_episodes + episode) + (district * 1000) + (int(day[-2:]) * 10000)
                        )  # Unique seed per run, episode, district, day
                        ...
                        # Extract KPIs
                        result = {"district": district, "day": day, "method": method, "run": run, "episode": episode}
                        for kpi in kpis:
                            result[kpi] = stats.get(kpi, 0)
                        run_results.append(result)

                    # Average KPIs over episodes within this run
                    avg_run_result = {"district": district, "day": day, "method": method, "run": run}
                    for kpi in kpis:
                ...
                avg_result = {"district": district, "day": day, "method": method}
                for kpi in kpis:
...
def create_plots(district_summary: pd.DataFrame, day_summary: pd.DataFrame, timestamp: str):
    ...
    # Line plot for on-time delivery rate
    sns.lineplot(
        data=day_summary, x="day", y="on_time_delivery_rate", hue="method", style="method", markers=True, ax=ax2
    )
